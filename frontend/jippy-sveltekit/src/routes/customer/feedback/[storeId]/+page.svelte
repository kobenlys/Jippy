<script>
    ...
    let complaintCategory = "실시간 매장 불편 사항";
    ...
    async function submitFeedback() {
      ...
      try {
        const response = await fetch(apiBaseUrl + "/api/feedback/" + data.storeId + "/create", {
          method: "POST",
          headers: {
  ...
  <div class="flex justify-center items-center min-h-screen bg-gray-100">
    <div class="bg-white w-full max-w-md sm:max-w-lg md:max-w-xl min-h-screen flex flex-col px-8 py-12 shadow-lg">
      
      <!-- 상단 영역 (제목) -->
      <div class="flex-none">
        <h2 class="text-2xl font-bold mb-6">고객 불편 사항</h2>
      </div>
      ...
      <div class="flex-grow flex flex-col space-y-6">
        <div class="relative">
          ...
          >
            <option>실시간 매장 불편 사항</option>
            <option>서비스 개선 요청</option>
            <option>기타</option>
          </select>
          <!-- 아이콘 (SVG 직접 추가) -->
          <svg class="absolute right-4 top-1/2 transform -translate-y-1/2 w-8 h-8 text-gray-500 pointer-events-none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
