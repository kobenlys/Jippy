...
public interface ChatRepository extends MongoRepository<StoreChat, Integer> {
    ...
    // 특정 timestamp보다 오래된 메시지를 가져오되, MongoDB에서 직접 필터링
    @Query(value = "{ 'storeId': ?0, 'messages.timestamp': { $lt: ?1 } }",
            sort = "{ 'messages.timestamp': -1 }")
    Optional<StoreChat> findOldMessages(Integer storeId, String before);
}
